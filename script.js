...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayQuestion(question, cardIndex) {
        ...
        questionTimer = setInterval(() => {
            ...
            if (timeLeft <= 0) {
                ...
                multipleChoiceOptionsContainer.querySelectorAll('button').forEach(b => {
                    b.onclick = null; // 移除舊的點擊事件
                    b.disabled = true; // 禁用按鈕
                });

                // 顯示全員搶答按鈕
                stealOptionsContainer.style.display = 'flex';
                stealOptionsContainer.innerHTML = ''; // 清空容器
                players.forEach((player, index) => {
                    const stealPlayerButton = document.createElement('button');
                    stealPlayerButton.textContent = `玩家 ${index + 1} 搶答`;
                    stealPlayerButton.onclick = () => {
                        handleSteal(question, cardIndex, index);
                    };
    ...
    function handleSteal(question, cardIndex, stealerIndex) {
        stealOptionsContainer.style.display = 'none'; // 隱藏搶答按鈕
        ...
        if (question.type === 'multiple_choice') {
            multipleChoiceOptionsContainer.querySelectorAll('button').forEach((button, optIndex) => {
                button.disabled = false; // 重新啟用選項按鈕給搶答者
                button.onclick = () => {
    ...
    function applyEventCardEffect(eventCard) {
        ...
        switch (eventCard.event_type) {
            case 'double_score':
                ...
                feedbackText = `玩家 ${currentPlayerIndex + 1} 分數加倍！從 ${oldScore} 點變為 ${player.score} 點！`; break;
            case 'swap_score':
                if (players.length > 1) {
            ...
            case 'random_score':
                ...
                feedbackText = `隨機事件！玩家 ${currentPlayerIndex + 1} 分數 ${randomPoints >= 0 ? `增加 ${randomPoints}` : `減少 ${-randomPoints}`} 點！`; break;
            ...
            default:
                ...
                feedbackText = `${eventCard.event_description} 玩家 ${currentPlayerIndex + 1} 點數變化：${points >= 0 ? '+' : ''}${points}`; break;
        }
